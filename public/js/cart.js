...
(function () {
  ...
  //遍历美食列表，给已经选择的套餐上checked属性
  for (var i in shopping_cart) {
    $('#food-' + shopping_cart[i].id).addClass('checked');

    //创建购物篮
    var dom = $(create_car_item(shopping_cart[i]));//调用了create_car_item方法
    ...
    if (el.hasClass('checked')) {//已经选中-> 取消
      ...
      for (var i in shopping_cart) {
        if (shopping_cart[i].id == el.attr('data-id')) {
          ...
          //重设购物车
          storage.setItem(shop_id, JSON.stringify(shopping_cart));//JSON.stringify将对象解析为json
        }
      }
    } else {//未选中
      el.addClass('checked');
  ...
  $('.buy-btn').click(function (e) {
    ...
    if (shopping_cart.length <= 0 && $('#picmenu>.ui-draggable').length==0) {
      ...
      //console.log(JSON.stringify(picmenu_list));//关于图片，这里应该是没有
  ...
  function changeNum() {
    ...
    for (var i in shopping_cart) {
      if (shopping_cart[i].id == food_id) {
        ...
        //重设购物车
        storage.setItem(shop_id, JSON.stringify(shopping_cart));
        //alert(JSON.parse(storage.getItem(shop_id))[1].num);
        //重新计算
        $("#cart_zongjia").text(get_total());
      }
    }
